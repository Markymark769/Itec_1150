...
def main():
    ...
    while True:
        if user_currency.upper() in rates['conversion_rates']:
            dollars = float(input('How many dollars to convert? '))
            conversion = rates['conversion_rates'][user_currency.upper()]
            print(f'As of {rates["time_last_update_utc"]}, ${dollars:,.2f} is about {conversion * dollars:,.2f} {user_currency.upper()}')
        else:
            print(f'{user_currency.upper()} is not a currency I can convert. I know about:\n')
            count = 0
            for currency in rates['conversion_rates']:
                count += 1
...
def get_exchange_rates():
    ...
    api_key = 'bcce1e6449d3ed9d57d271a3'
    url = f'https://v6.exchangerate-api.com/v6/{api_key}/latest/USD'
    # 'https://v6.exchangerate-api.com/v6/bcce1e6449d3ed9d57d271a3/latest/USD'
