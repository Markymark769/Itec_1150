...
weather_word_document.add_paragraph('Current Weather Overview', 'Title')
# create a loop that goes over the list of weather conditions within API call
for hourly_data in list_of_weather_conditions:
    if weather.items():  # if the data in the hourly key exists, put it in an empty list called weather
        # below we are specific again on what data to pull out.
        time = hourly_data['time']  # we would like the hourly time
        print(list_of_weather_conditions)

        weather_word_document.add_paragraph('Time', 'Heading 1')
        weather_word_document.add_paragraph(time)
    ...
    #     weather_word_document.add_paragraph(f'The hourly data for time is the following:{time}')
    #     weather_word_document.add_paragraph(f'The hourly data for the temperature is the following: {temperature_2m}')
    #     weather_word_document.add_paragraph(f'The hourly data for humidity is the following: {relative_humidity_2m}')
    #     weather_word_document.add_paragraph(f'The hourly data for wind speed is the following: {wind_speed_10m}')
...
#         print(time)
#         url_for_time = 'https://api.open-meteo.com/v1/forecast?latitude=52.52&longitude=13.41&current=temperature_2m,wind_speed_10m&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m' + time
#         print(url_for_time)
#         url_for_detail_time = requests.get(url_for_time).json()
#         print(url_for_detail_time)


weather_word_document.save('Current_Weather.docx')
