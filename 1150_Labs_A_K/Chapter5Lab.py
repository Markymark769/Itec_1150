...
def display_menu():
    ...
    print('edit - Edit country name or code')
...
def edit(countries):
    display_codes(countries)
    country_code = input('Enter country code to edit: ')
    country_code = country_code.upper()
    if country_code in countries:
        country_name = countries[country_code]
        print('Country name: ' + country_name + '.\n')
        new_country_code = input('What is the country code? ')
        new_country_name = input('What is the country name? ')
        del countries[country_code]
        countries.update({new_country_code.upper(): new_country_name.title()})
    else:
        print('There is no country with that code. \n')
    return(countries)
