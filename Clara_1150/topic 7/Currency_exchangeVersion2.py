...
while True:
    ...
    currency_name = input('Enter the currency code listed to exchange: ')
    if currency_name not in rates:
        print('Currency code that not in the dictionary, conversion can\'t be done.')
    else:
        how_many_euros = float(input('How many Euros to convert? '))
        # more_names = input('Any more currency to exchange? Y for more, N to stop')
        # if more_names == 'N':
        #     break
