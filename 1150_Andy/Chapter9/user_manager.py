"""
Mark Porraz, 4/4/2024, user_manager.py
This program initializes a text file to store user information with at least one line.
Each line in the file will contain a username and an email address, separated by space
(example: "egranse  egranse@minnstate.edu"). No spaces are allowed in the username.

using pyinput plus, this program will create a menu which allow the user to:
    View: read the contents of the user file and display all users as shown in the sample
    on the next slide.
    Add:
...
def main():
    try:
        user_file = open('user_manager.txt', 'w')  # opens the directory txt file in write mode
        user_file.write('mmouse mickeymouse@gmail.com\n')  # writes a preloaded Username
...
def view():
    user_file = open('user_manager.txt', 'r')  # opens the directory file
    directory = user_file.read()  # reads the directory file
    print(directory)
    user_file.close()  # close is usually the last function
...
def add():
    ...
    directory_file = open('user_manager.txt', 'a')  # appends/writes to the dictionary txt
