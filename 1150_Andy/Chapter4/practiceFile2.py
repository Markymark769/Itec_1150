...
def main():  # call functions and save results under key variable names.
    try:  # generic exception handling - turn off during development
        book_name, num_books, price_list = inputs()
        total, average = processing(price_list)
        outputs(num_books, book_name, price_list, total, average)
...
def inputs():  # collect info needed from the user.
    ...
    book_title = input('Enter the title of the book:')  # user sets the list length of books
    book_name = get_book_title(book_title)  # call for validation to collect names of book
    ...
    return book_name, num_books, price_list  # what is expected to return, must be in the order called
...
def outputs(num_books, book_name, price_list, total, average): # display information about each book, and summary info
